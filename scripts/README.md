...
| 068 | `068-catalogos-unique-por-tenant.sql` | **Aislamiento multi-tenant de catálogos.** Cambia la unicidad de `categorias` y `marcas` de un **UNIQUE global sobre `nombre`** a un índice único **por empresa** `(razon_social_id, lower(btrim(nombre)))`. Corrige que dos empresas no pudieran tener catálogos con el mismo nombre (rompía la carga masiva / creación al vuelo). Antes de crear el índice, borra duplicados exactos por empresa **sin uso** (sin productos ni subcategorías/marcas colgando); luego suelta el constraint/índice UNIQUE global viejo (descubierto dinámicamente). Atómico (`BEGIN/COMMIT`), idempotente. `subcategorias` no se toca (ya estaba aislada). Es un `DROP`/`CREATE INDEX` que **no reescribe filas**: lock breve de ms con los volúmenes actuales. |
