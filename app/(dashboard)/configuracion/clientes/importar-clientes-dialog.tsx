...
import {
  parsearArchivoClientes, previsualizarImportClientes, importarClientes, descargarPlantillaClientes,
  type FilaClienteImport, type PreviewClientes, type ResultadoImportClientes,
} from "@/lib/services/importar-clientes"

export function ImportarClientesDialog({ onImported }: { onImported: () => void }) {
  const { toast } = useToast()

  const [open, setOpen] = React.useState(false)
  const [filas, setFilas] = React.useState<FilaClienteImport[]>([])
  const [nombreArchivo, setNombreArchivo] = React.useState("")
  const [preview, setPreview] = React.useState<PreviewClientes | null>(null)
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setResultado(null)
    try {
      const parsed = await parsearArchivoClientes(file)
      if (parsed.length === 0) {
        toast({ title: "Archivo vacío", description: "No se encontraron clientes válidos.", variant: "destructive" })
        resetArchivo()
        return
      }
      setFilas(parsed)
      setNombreArchivo(file.name)
      setPreview(await previsualizarImportClientes(parsed))
    } catch {
  ...
  async function ejecutar() {
    ...
    try {
      ...
      if (res.data.creados > 0) {
        toast({ title: "Carga completada", description: `${res.data.creados} cliente(s) creado(s).` })
        onImported()
      }
    } finally {
      setImportando(false)
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!importando) { setOpen(o); if (!o) resetArchivo() } }}>
      ...
      <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Descarga la plantilla, complétala con tus clientes y súbela. Se omiten los que ya existen (por RTN o nombre).
          </DialogDescription>
        </DialogHeader>
            ...
            {resultado.clientes.some((c) => c.estado !== "creado") && (
              <ScrollArea className="h-40 rounded-md border">
                <div className="p-2 space-y-1">
                  {resultado.clientes.filter((c) => c.estado !== "creado").map((c, i) => (
                    <div key={i} className="flex items-center gap-2 text-xs">
                      <Badge
                        variant="secondary"
                        className={c.estado === "omitido" ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}
                      >
                        {c.identificador}
                      </Badge>
                      <span className="text-stone-600">{c.detalle}</span>
                    </div>
                  ))}
                </div>
              </ScrollArea>
